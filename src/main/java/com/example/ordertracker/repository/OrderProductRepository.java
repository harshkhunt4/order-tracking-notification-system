...
public interface OrderProductRepository extends JpaRepository<OrderProduct, OrderProductPK> {
  ...
  @Query("SELECT op FROM OrderProduct op WHERE op.status <> :status ORDER BY op.pk.order.createAt DESC")
  List<OrderProduct> findTop5StatusIsNot(@Param("status") OrderStatus status,Pageable pageable);
}
